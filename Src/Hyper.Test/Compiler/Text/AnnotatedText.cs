...
internal sealed class AnnotatedText
{
    ...
    public static AnnotatedText Parse(string text)
    {
        ...
        foreach (var c in text)
        {
            ...
            else if (c == ']')
            {
                ...
                var end   = position;
                var span  = TextSpan.MakeTextSpanFromBound(start, end);
    ...
    private static string ReduceIndent(string text)
    {
        var lines = ReduceIndentWithLines(text);
        return string.Join(Environment.NewLine, lines);
    }

    public static string[] ReduceIndentWithLines(string text)
    {
